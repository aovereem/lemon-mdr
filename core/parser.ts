...
export function parseLine(raw: string, fileSessionId: string): Fact | null {
  ...
  // session (keyed by the file's agent id), forked from the parent session.
  ...
  // Bookkeeping lines carry no activity — skip before they become noise.
  if (
    entry.type === "summary" ||
  ...
  // Count only newly generated output + genuinely new (uncached) input. NEVER add
  // cache_read/cache_creation — each turn re-sends the whole context and those balloon
  // into the hundreds of millions of phantom tokens.
  ...
  // "end_turn" = the model finished. Only the final text entry carries it (a turn's last
  // message is split into a thinking entry + a text entry, both stamped end_turn — flag
  // just the text one, else the turn seals twice).
  ...
  // A "user" line is either a genuine prompt (new turn) or tool output coming back.
  // Tool output carries tool_result blocks; a real prompt does not.
  if (entry.type === "user") {
...
// Defensive/verification work: known test+lint+typecheck runners, or a package script
// named test/lint/typecheck/check/ci. Read straight from the Bash command — never inferred.
...
// Lines a tool changed, counted from the tool's OWN arguments — never read off disk
// (keeps the visualizer passive). A 10→10 rewrite reads as +10/−10 (churn).
...
/** Knowledge-gathering tools — scouting (frequent, drives the Dread temper via reads). */
